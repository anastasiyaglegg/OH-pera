import {trustedUrl,sourceHosts,listingUrl} from '../../lib/security.ts';
...
export function safeUrl(value,base){return trustedUrl(value,[...sourceHosts,'www.universe.com','universe.com'],base);}
...
export function event(source, fields, checkedAt) {
  if(!validDate(fields.date) || !fields.title?.trim() || !listingUrl(fields.sourceUrl,source.id)||(fields.ticketUrl!=null&&!listingUrl(fields.ticketUrl,source.id,'ticket'))) throw new Error('Invalid performance record');
