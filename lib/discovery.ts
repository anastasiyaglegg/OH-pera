...
// Shared discovery season, September through August; individual company seasons may differ.
export function seasonEnd(today:string){const year=Number(today.slice(0,4));return `${Number(today.slice(5,7))>=9?year+1:year}-08-31`;}
export function matchesDate(date:string,today:string,range:string,chosen:string){if(date<today)return false;const day=(d:string)=>Date.parse(d+'T12:00:00Z');const diff=(day(date)-day(today))/86400000;if(range==='week')return diff<7;if(range==='season')return date<=seasonEnd(today);if(range==='month')return date.slice(0,7)===today.slice(0,7);if(range==='date')return !!chosen&&date===chosen;if(range==='weekend'){const weekday=new Date(day(today)).getUTCDay();const start=weekday===0?0:(6-weekday+7)%7;return diff>=start&&diff<=start+(weekday===0?0:1);}return true;}
export type Filters={q:string;range:string;date:string;kind:string;company:string;venue:string;borough:string};
export const defaultFilters:Filters={q:'',range:'any',date:'',kind:'all',company:'all',venue:'all',borough:'all'};
export function filterEvents(events:Performance[],f:Filters,today:string){const query=normalize(f.q);return events.filter(p=>matchesDate(p.date,today,f.range,f.date)&&(f.kind==='all'||p.kind===f.kind)&&(f.company==='all'||p.company===f.company)&&(f.venue==='all'||p.venue===f.venue)&&(f.borough==='all'||p.borough===f.borough)&&normalize([p.title,p.composer,p.company,p.venue].filter(Boolean).join(' ')).includes(query));}
