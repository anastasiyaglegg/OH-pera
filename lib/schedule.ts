export type Performance = {
 ...
 provenanceUrl?:string;verification?:'secondary';
};
...
export function isStale(date:string|null,now=Date.now()):boolean {return !date||!Number.isFinite(Date.parse(date))||now-Date.parse(date)>24*60*60*1000;}
